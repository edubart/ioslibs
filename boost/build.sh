...
: ${BOOST_LIBS:="random regex random chrono thread signals filesystem system date_time"}
...
copyBuild()
{
    ...
    mkdir -p $IOSDIR/{lib,include}
    cp $FRAMEWORK_INSTALL_NAME $IOSDIR/lib/libboost.a
    cp -r $PREFIXDIR/include/boost  $IOSDIR/include/boost
}
